...
export async function POST(req: Request) {
  ...
  const { id, title, description, assigned_to, state, priority } = body;
  // Delete existing if same id
  if (id) await sql`DELETE FROM hq_tasks WHERE id = ${id}`;
  const rows = await sql`
    INSERT INTO hq_tasks (id, title, description, assigned_to, state, priority, updated_at)
    VALUES (${id ?? null}, ${title}, ${description ?? null}, ${assigned_to ?? null}, ${state ?? 'backlog'}, ${priority ?? 'should'}, NOW())
    RETURNING *
  `;
